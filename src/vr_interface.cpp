      ...
      max_devices_(vr::k_unMaxTrackedDeviceCount)  // or vr::k_unMaxTrackedDeviceCount
{
  ...
  play_area_[1] = -1;
  for (int i = 0; i < 4; i++)
    for (int o = 0; o < 3; o++) play_quat_.vCorners[i].v[o] = -1;

  return;
}

VRInterface::~VRInterface() { return; }
...
void VRInterface::Update() {
  if (pHMD_) {
    pHMD_->GetDeviceToAbsoluteTrackingPose(vr::TrackingUniverseRawAndUncalibrated, 0, device_poses_,
                                           max_devices_);
...
int VRInterface::GetDeviceMatrix(int index, double pMatrix[3][4]) {
  if (index < max_devices_) {
    if (device_poses_[index].bDeviceIsConnected && device_poses_[index].bPoseIsValid &&
      ...
      for (int i = 0; i < 3; i++)
        for (int o = 0; o < 4; o++)
          pMatrix[i][o] =
              static_cast<double>(device_poses_[index].mDeviceToAbsoluteTracking.m[i][o]);
...
void VRInterface::UpdateCalibration() {
  ...
  if (!pChaperone_->GetPlayAreaRect(&play_quat_))
    for (int i = 0; i < 4; i++)
      info_("Corner " + std::to_string(i) + " - x: " + std::to_string(play_quat_.vCorners[i].v[0]) +
            ", y: " + std::to_string(play_quat_.vCorners[i].v[1]) +
            ", z:" + std::to_string(play_quat_.vCorners[i].v[2]));
}

std::string VRInterface::GetTrackedDeviceString(vr::IVRSystem*            pHmd,
...
void VRInterface::HandleInput(vr::TrackedDeviceIndex_t unControllerDeviceIndex,
                              vr::VRControllerState_t& state) {
  // Process SteamVR controller state
  pHMD_->GetControllerState(unControllerDeviceIndex, &state, sizeof(vr::VRControllerState_t));
}

void VRInterface::TriggerHapticPulse(vr::TrackedDeviceIndex_t unControllerDeviceIndex,
                                     uint32_t unAxisId, int usDurationMicroSec) {
